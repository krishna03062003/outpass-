const MessModel = require("../model/meshbillmodel");
...
const setMessRate = async (req, res) => {
  try {
    ...
    if (!startDate || !endDate || !monthlyRate||!semfees||!elect||!water||!serventsalary ) {
      return res.status(400).json({ message: "All fields are required" });
    }

    let setting = await MessModel.findOne().sort({ createdAt: -1 });
...
const getAllMessBills = async (req, res) => {
    try {
      const setting = await MessModel.findOne().sort({ createdAt: -1 });
